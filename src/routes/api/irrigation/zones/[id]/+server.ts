...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
  ...
  try {
    ...
    if (err instanceof Error && /UNIQUE constraint failed/i.test(err.message)) {
      return json(
        { ok: false, error: `Station ${patch.stationSid} is already used by another zone` },
        { status: 409 }
      );
    }
    throw err;
  ...
  if (getOpenSprinklerConfig().enabled) {
    ...
    if (zone.stationSid !== existing.stationSid)
      getIrrigationController().retractStation(existing.stationSid);
