...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (live.saturated)
    throw error(409, 'Live frame is saturated — dim the light or shorten exposure, then retry');
  ...
  if (source === 'lux') {
    ...
    if (!Number.isFinite(umol) || umol <= 0) {
      ...
      if (!live_ppfd)
        throw error(409, 'No live Apogee reading available — pass an explicit `referenceUmol`');
      umol = live_ppfd.ppfd;
      meter = 'apogee-sq521';
...
export const DELETE: RequestHandler = async ({ url }) => {
  ...
  if (source !== 'lux' && source !== 'reference')
    throw error(400, 'source must be lux or reference');
