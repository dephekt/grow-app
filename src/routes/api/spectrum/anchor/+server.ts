...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (!live) error(404, 'No live spectrum to anchor against');
  if (live.saturated)
    error(409, 'Live frame is saturated — dim the light or shorten exposure, then retry');
  ...
  if (source === 'lux') {
    ...
    if (!Number.isFinite(lux) || lux <= 0) {
      ...
      if (!live_lux) error(409, 'No live illuminance available — pass an explicit `lux`');
      lux = live_lux.lux;
      meter = 'bh1750-dlight';
    }
    if (!(lux > 0)) error(400, 'lux must be a positive number');
    anchor = luxToAnchor(live.counts, live.integrationUs, lux, {
      capturedAt,
      meter,
    ...
    if (!Number.isFinite(umol) || umol <= 0) {
      ...
      if (!live_ppfd)
        error(409, 'No live Apogee reading available — pass an explicit `referenceUmol`');
      umol = live_ppfd.ppfd;
      meter = 'apogee-sq521';
    ...
    if (!(umol > 0)) {
      error(
        400,
        meter
          ? 'Live Apogee reading is not positive (sensor dark?) — anchor under light'
          : 'referenceUmol must be a positive number'
      );
    }
...
export const DELETE: RequestHandler = ({ url }) => {
  ...
  if (source !== 'lux' && source !== 'reference') error(400, 'source must be lux or reference');
