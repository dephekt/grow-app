  ...
  try {
    ...
    if (!updateEntity) {
      return json(
        {
          ok: false,
          error: 'Firmware update entity is not discovered; bootstrap this device first'
        },
        { status: 409 }
      );
    }
    ...
    if (!resolved)
      return json(
        { ok: false, error: `No ${channel} firmware package found for ${device.package}` },
        { status: 404 }
      );
    if (requestedVersion && requestedVersion !== resolved.manifest.version) {
      return json(
        { ok: false, error: 'Requested firmware version is no longer selected' },
        { status: 409 }
      );
    }
    ...
    if (resolveInstalledVersion(updateState, device) === resolved.manifest.version) {
      return json(
        { ok: false, error: 'Selected firmware version is already installed' },
        { status: 409 }
      );
    }
