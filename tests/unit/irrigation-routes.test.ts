...
const { PATCH: PROBE_PATCH, DELETE: PROBE_DELETE } =
  await import('../../src/routes/api/irrigation/probes/[nodeId]/+server');
...
beforeEach(() => {
  getIrrigationDb().exec(
    'DELETE FROM substrate_probes; DELETE FROM zones; DELETE FROM irrigation_events;'
  );
});
...
describe('/api/irrigation/events', () => {
  function insertEvents(count: number): void {
    ...
    for (let i = 0; i < count; i++)
      insert.run(i, `2026-08-10T10:${String(i).padStart(2, '0')}:00.000Z`);
  }

  const getEvents = (query = '') =>
    EVENTS_GET({ url: new URL(`http://localhost/api/irrigation/events${query}`) } as Parameters<
      typeof EVENTS_GET
    >[0]) as Response;
...
describe('/api/irrigation/probes/[nodeId]', () => {
  ...
  const patch = (opts: Parameters<typeof probeEvent>[0]) =>
    PROBE_PATCH(
      probeEvent(opts) as unknown as Parameters<typeof PROBE_PATCH>[0]
    ) as Promise<Response>;
